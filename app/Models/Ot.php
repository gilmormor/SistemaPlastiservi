...
class Ot extends Model
{
    ...
    //Relacion inversa a Usuario quien aprobo el registro
    public function aprobusu()
    {
        return $this->belongsTo(Usuario::class);
    }
    //RELACION de uno a uno otnotaventa
    public function otnotaventa()
    {
        return $this->hasOne(OtNotaVenta::class);
    }    
    //RELACION de uno a uno otanul
    public function otanul()
    {
        return $this->hasOne(OtAnul::class);
    }

    //RELACION DE UNO A MUCHOS OtOc
    public function otocs()
    {
        return $this->hasMany(OtOc::class,'ot_id');
    }
    
    //RELACION DE UNO A UNO OtOc
    public function otoc()
    {
        return $this->hasOne(OtOc::class,'ot_id');
    }
    ...
    public static function reportOtItem($request){
        $user = Usuario::findOrFail(auth()->id());
        ...
        if(!isset($request->oc_id) or empty($request->oc_id)){
            ...
            $aux_condoc_id = "(otoc.oc_id='$request->oc_id' or notaventa.oc_id='$request->oc_id')";
            $aux_condFecha = " true";
        }
        if(!isset($request->notaventa_id) or empty($request->notaventa_id)){
            $aux_condnotaventa_id = " true";
        ...
        if(!isset($request->sucursal_id) or empty($request->sucursal_id) or ($request->sucursal_id == "")){
            ...
            $aux_sucursal_idCond = "ot.sucursal_id in ($request->sucursal_id)";
        }
        $aux_centroeconomico_idCond = " true ";
        /* if(!isset($request->centroeconomico_id) or empty($request->centroeconomico_id) or ($request->centroeconomico_id == "")){
            $aux_centroeconomico_idCond = "true";
        ...
        if(!isset($request->materiaprima_id) or empty($request->materiaprima_id) or ($request->materiaprima_id == "")){
            $aux_materiaprima_idCond = "true";
        }else{
            $aux_materiaprima_idCond = "acuerdotecnico.at_materiaprima_id in ($request->materiaprima_id)";
        }

        if(!isset($request->producto_id) or empty($request->producto_id) or ($request->producto_id == "")){
            $aux_producto_idCond = "true";
        }else{
            $aux_producto_idCond = "otdet.producto_id in ($request->producto_id)";
        }

        if(!isset($request->at_AnchoIni) or !isset($request->at_AnchoFin) or empty($request->at_AnchoIni) or empty($request->at_AnchoFin)){
            $aux_condAncho = " true";
        }else{
            $at_AnchoIni = $request->at_AnchoIni;
            $at_AnchoFin = $request->at_AnchoFin;
            $aux_condAncho = "acuerdotecnico.at_ancho>='$at_AnchoIni' and acuerdotecnico.at_ancho<='$at_AnchoFin'";
        }
        if(!isset($request->sta_envprog)){
            ...
            /* $aux_filtrootcProgramar = "otdet.id not in (
                                                            ...
                                                            AND op.id not in 
                                                            (
                                                                SELECT opanul.op_id from opanul WHERE isnull(opanul.deleted_at)
                                                            )
                                                        )"; */
            $aux_filtrootcProgramar = "otdet.id not in (
                                                            SELECT otdet_id FROM otdetcerr WHERE isnull(otdetcerr.deleted_at)
                                                        ) 
                                                        AND ot.id not in 
                                                        (
                                                            SELECT otcerr.ot_id from otcerr WHERE isnull(otcerr.deleted_at)
                                                        )
                                                        AND ot.id not in 
                                                        (
                                                                SELECT otanul.ot_id from otanul WHERE isnull(otanul.deleted_at)
                                                        )";

        }

        $aux_sta_fabricacionCond = "true";        
        if(isset($request->sta_fabricacion) and $request->sta_fabricacion == 1){
            $aux_sta_fabricacionCond = " otdet.requiere_fabricacion = 1";
        }
        ...
        $sql = "SELECT otdet.id,ot.id as ot_id,ot.fechahora,cliente.razonsocial,cliente.rut,ot.cliente_id,
        IFNULL(otoc.oc_id,notaventa.oc_id) as oc_id,IFNULL(otoc.oc_file,notaventa.oc_file) as oc_file,
        IFNULL(otoc.oc_id,null) as staus_oc,ot.aprobstatus,
        comuna.nombre as comuna_nombre,
        UNIX_TIMESTAMP(otdet.updated_at) as updatednum_at,otdet.updated_at,
        UNIX_TIMESTAMP(ot.updated_at) as otupdatednum_at,ot.updated_at,
        clientebloqueado.descripcion as clientebloqueado_descripcion,
        if(cliente.plazopago_id = 1,'Condición pago: Contado',clientebloqueado.descripcion) as clientebloqueado_desc,
        cliente.limitecredito,
        IFNULL(vista_datacobranza.tfac,0) AS datacobranza_tfac,
        ...
        AND $aux_condAncho
        AND $aux_sta_envprogCond
        AND isnull(ot.deleted_at)
        AND $aux_filtrootcProgramar
        AND $aux_sta_fabricacionCond
        GROUP BY otdet.id;";
        ...
        foreach ($datas as &$data) {
            $productoarray = Producto::atributosProducto($data->producto_id);
            $data->producto_nombre = $productoarray["nombre"];
            $data->cla_nombre = $productoarray["cla_nombre"];
            $data->at_sello = $productoarray["cla_nombre"];
            //dd($data);
        }
