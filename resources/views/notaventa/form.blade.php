        ...
        -->
            ...
            <div class="row">
                ...
                <div class="form-group col-xs-12 col-sm-3">
                    ...
                    <input type="text" name="contacto" id="contacto" class="form-control" value="{{old('contacto', $data->contacto ?? '')}}" required placeholder="Contacto Entrega"/>
                </div>
                <div class="form-group col-xs-12 col-sm-3">
                    <label for="contactoemail" class="control-label requerido">Email</label>
                    <input type="email" name="contactoemail" id="contactoemail" class="form-control" value="{{old('contactoemail', $data->contactoemail ?? '')}}" required placeholder="Email Contacto Entrega"/>
                </div>
                <div class="form-group col-xs-12 col-sm-3">
                    <label for="contactotelf" class="control-label requerido">Teléfono</label>
                    <input type="text" name="contactotelf" id="contactotelf" class="form-control" value="{{old('contactotelf', $data->contactotelf ?? '')}}" required placeholder="Teléfono Contacto Entrega"/>
                </div>
                <div class="form-group col-xs-12 col-sm-3">
                    <label for="observacion" class="control-label">Observaciones</label>
                    <input type="text" name="observacion" id="observacion" class="form-control" value="{{old('observacion', $data->observacion ?? '')}}" placeholder="Observaciones"/>
                </div>
            </div>
        <div class="row">
            <div class="form-group col-xs-12 col-sm-3">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="card" style="width: 87rem">
                                <?php
                                    $info = new SplFileInfo($data->oc_file);
                                    $ext = $info->getExtension();
                                    if($ext=='pdf'){
                                        ?>
                                        <object data="{{Storage::url("imagenes/notaventa/$data->oc_file")}}" type="application/pdf" width="100%" height="100%"></object>
                                    <?php }
                                    else{
                                    ?>
                                        <img id="id_img_fondo" src="{{Storage::url("imagenes/notaventa/$data->oc_file")}}" alt="User Image" >
                                    <?php }
                                ?>
                            </div>
                        </div>
