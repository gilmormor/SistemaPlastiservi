...
@section('scripts')
    ...
    <script src="{{autoVer("assets/pages/scripts/cotizacionatfirmado/crear.js")}}" type="text/javascript"></script>
@endsection
    ...
    <div class="col-lg-12">
        ...
        <div class="box box-primary">
            ...
            <form action="{{route('actualizar_cotizacionatfirmado', ['id' => $data->id])}}" id="form-general" class="form-horizontal" method="POST" autocomplete="off" enctype="multipart/form-data">
                @csrf @method("put")
                <div class="box-body">
                    @include('cotizacionatfirmado.form')
                </div>
                <!-- /.box-body -->
                ...
                <div class="box-footer text-center">
                    @if (session('aux_aprocot')=='0')
                        @if (($data->vendedor_id == $tablas['vendedor_id']) or ($data->usuario_id == auth()->id()) or ($tablas['usuario']->persona->sta_editarnvcot == 1)) <!-- Solo deja modificar si es el mismo vendedor o si fue el usuario que creo el registro or si es el usuario administrador-->
