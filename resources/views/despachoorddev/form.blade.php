...
<div class="row">
    <div class="col-xs-12 col-sm-12">
        @include('despachoorddev.datosform')
    </div>    
</div>
                    ...
                    <tbody>
                        @if ($aux_sta==2 or $aux_sta==3)
                            ...
                            @foreach($detalles as $detalle)
                                <?php
                                    //dd($detalles);
                                    $sql = "SELECT despachoorddet_id,cantdev
                                            FROM vista_sumdevorddespdet
                                    ...
                                    //dd($despachoorddevdets);
                                    if(empty($datasuma)){
                                        $sumacantorddespdev= 0;
                                    }else{
                                        $sumacantorddespdev= $datasuma[0]->cantdev;
                                    }
                                    $aux_cantdev = 0;
                                    $aux_subtotalkg = 0.00;
                                    ...
                                    $aux_checked = "";
                                    $aux_despachoorddevdet_id = "";
                                    if($aux_sta==3){
                                        foreach($despachoorddevdets as $despachoorddevdet){
                                            if($detalle->id == $despachoorddevdet->despachoorddet_id){
                                                //dd($sumacantorddespdev);
                                                //$sumacantorddespdev = $sumacantorddespdev - $despachoorddevdet->cantdev;
                                                $sumacantorddespdev = $sumacantorddespdev - $despachoorddevdet->cantdev;
                                                $aux_cantdev = $despachoorddevdet->cantdev;
                                                $aux_subtotalkg = $despachoorddevdet->cantdev * $peso;
                                                $aux_subtotalCFTD = $despachoorddevdet->cantdev * $detalle->notaventadetalle->preciounit;
                                                $aux_cantdevTotal += $despachoorddevdet->cantdev;
                                                $aux_checked = "checked";
                                                $aux_despachoorddevdet_id = $despachoorddevdet->id;
                                            }
                                        }                                        
                                    }
                                    if($detalle->cantdesp > $sumacantorddespdev){
                                        $aux_nfila++;
                                        $aux_saldo = $detalle->cantdesp - $sumacantorddespdev - $aux_cantdev;
                                ?>
                                        <tr name="fila{{$aux_nfila}}" id="fila{{$aux_nfila}}">
                                            ...
                                            <td style="display:none;">
                                                ...
                                                <input type="text" name="despachoorddevdet_id[]" id="despachoorddevdet_id{{$aux_nfila}}" class="form-control" value="{{$aux_despachoorddevdet_id}}" style="display:none;"/>
                                            </td>
                                            <td style="display:none;">
                                            ...
                                            <td name="cantordF{{$aux_nfila}}" id="cantordF{{$aux_nfila}}" style="text-align:right">
                                                <input type="text" name="cantord[]" id="cantord{{$aux_nfila}}" class="form-control numerico cantordsum" onkeyup="actSaldo({{$aux_nfila}})" value="{{$aux_cantdev}}" style="text-align:right;"/>
                                            </td>
                                            <td name="cantorddespinputF{{$aux_nfila}}" id="cantorddespinputF{{$aux_nfila}}" style="text-align:right;display:none;">
                                                <input type="text" name="cantorddesp[]" id="cantorddesp{{$aux_nfila}}" class="form-control" value="{{$aux_cantdev}}" style="text-align:right;"/>
                                            </td>
                                            <td style="display:none;">
                            ...
                            <tr id="trneto" name="trneto">
                                ...
                                <td style="text-align:right">
                                    <div class="form-group col-xs-12 col-sm-12" style="margin-bottom: 0px;padding-left: 0px;padding-right: 0px;">
                                        <input type="text" name="cantordTotal" id="cantordTotal" class="form-control" style="text-align:right;" value="{{$aux_cantdevTotal}}" readonly required/>
                                    </div>
                                </td>
