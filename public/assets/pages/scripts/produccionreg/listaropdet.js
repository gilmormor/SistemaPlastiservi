$(document).ready(function () {
    ...
    let table =$('#tabla-data-consulta').DataTable({
        ...
        'processing'  : true,
        'serverSide'  : true,
        'ajax'        : "/produccionreg/listaropdetpage/" + data.data2, //$("#annomes").val() + "/sucursal/" + $("#sucursal_id").val(),
        "order": [[ 0, "desc" ]],
        'columns'     : [
            {
        ...
        "createdRow": function ( row, data, index ) {
            ...
            aux_text = 
            `<a class="btn-accion-tabla tooltipsC enlace-soldesp botonac${data.id}" title="Procesar Registro Produccion" href="/produccionreg/crearini/${data.id}/${data.updatednum_at}">
    ...
    {
        data = datosopdet();
        $('#tabla-data-consulta').DataTable().ajax.url( "/produccionreg/listaropdetpage/" + data.data2 ).load();
        //totalizar();
    });
