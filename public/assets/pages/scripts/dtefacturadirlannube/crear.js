...
function calsubtotalitem(name){
	...
	let qtyitem = $("#qtyitem" + i).val() == "" ? 0.00 : parseFloat($("#qtyitem" + i).val());
	let prcitem = $("#prcitem" + i).val() == "" ? 0.00 : parseFloat($("#prcitem" + i).val());
	...
	aux_subtotal = aux_subtotal.toFixed(2); // Math.round(aux_subtotal,2);
...
function totalizarlannube(){
	...
	if(aux_foliocontrol_id == 7){
		...
		aux_iva = aux_iva.toFixed(2);
	}
	aux_total = parseFloat(total_neto) + parseFloat(aux_iva);
	aux_netoform = MASKLA(total_neto,2); //MASK(0, total_neto, '-#,###,###,##0.00',1)
	aux_ivaform = MASKLA(aux_iva,2); //MASK(0, aux_iva, '-#,###,###,##0.00',1)
	aux_tdtotalform = MASKLA(aux_total,2); //MASK(0, aux_total, '-#,###,###,##0.00',1)
