$(document).ready(function () {
    ...
    function configurarTabla(aux_tabla){
        ...
        $(aux_tabla).DataTable({
            ...
            'serverSide'  : true,
            "order"       : [[ 0, "desc" ]],
            'ajax'        : "/reportdteestadocli/reportdteestadoclipage/" + data.data2, //$("#annomes").val() + "/sucursal/" + $("#sucursal_id").val(),
            'columns'     : [
                {data: 'id'}, // 0
                {data: 'fechahora'}, // 1
                {data: 'rut'}, // 2
    ...
    {
        data = datosFac();
        $('#tabla-data-consulta').DataTable().ajax.url( "/reportdteestadocli/reportdteestadoclipage/" + data.data2 ).load();
    });
});
