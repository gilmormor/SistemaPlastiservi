$(document).ready(function () {
	...
	$(".numerico").numeric({ negative : false, decimalPlaces: 2 });
	...
	// Si hay un valor inicial, configúralo en el datepicker
	/* if (initialDate) {
		$("#fechaestdesp").datepicker("setDate", new Date(initialDate));
	} */
	...
	$(".itemkgotnv").blur(function(e){
		if($(this).attr('valor') != undefined){
			i = $(this).attr('item');
			let aux_valor = $(this).attr("valor");
			if(aux_valor == ""){
				aux_valor = "0";
			}
			$("#totalkilos" + i).val(aux_valor);
			$("#totalkilos" + i).attr('valor',aux_valor);
		}
		totalizatotnv();
	});

	$(".itemkgotnv").on("input", function () {
    ...
    $(".itemkgotnv").on("keydown", function (event) {
...
function insertarTabla(){
    ...
    var htmlTags = '<tr name="fila'+ aux_nfila + '" id="fila'+ aux_nfila + '">'+
			...
			'<td name="precioxkiloTD'+ aux_nfila + '" id="precioxkiloTD'+ aux_nfila + '" style="text-align:right">'+ 
				MASKLA(aux_precioxkilo,3)+ //MASK(0, aux_precioxkilo, '-##,###,##0.00',1)+
			'</td>'+
			'<td style="text-align:right;display:none;">'+ 
		...
		'<tr id="triva" name="triva">'+
			'<td colspan="8" style="text-align:right"><b>IVA ' + $("#aux_iva").val() + '%</b></td>'+
...
function clickstafab(id){
	//console.log(id);
	estaSeleccionado = $("#aux_stafab" + id).is(":checked");
    $("#stafab" + id).val('0');
    if(estaSeleccionado){
        $("#stafab" + id).val('1');
    }
...
$(".editarcampoTex").click(function()
{
	...
	$("#lbleditarcampo").html($(this).attr('data-original-title') + " Id Producto: " + $("#producto_id" + id).val());
	$("#myModalEditarCampoTex").modal('show');
});

$('#myModalEditarCampoTex').on('shown.bs.modal', function () {
    $("#auxeditcampoT").focus();
});
...
$("#btnaceptarMT").click(function(event){
	...
	if(verificarDato(".valorrequerido"))
	{
		...
		if($("#auxeditcampoT").attr('aux_nomcampot') == "campo_obs"){
			$("#otdet_obs" + id).val(auxeditcampoT);
			//$("#obs" + id).html(auxeditcampoT);
			$("#campo_obs" + id).attr('valor', auxeditcampoT);
			$("#nmbitem" + id).val(auxeditcampoT);
			$("#nombreProdTD" + id).attr('title', auxeditcampoT);
		}
...
function totalizatotnv(){
	total_neto = 0;
	total_kg = 0;
	total_cant = 0;
	$("#tabla-data tr .totalkilos").each(function() {
		//valor = $(this).html() ;
