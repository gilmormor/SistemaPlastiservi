...
function ajaxRequest(data,url,funcion) {
	...
	$.ajax({
		...
		success: function (respuesta1) {
            if(funcion=='consulta'){
                $("#TDeuda").val(MASKLA(0,0));
                $("#TDeudaFec").val(MASKLA(0,0));
                $('#tabla-data-consulta').DataTable().clear();
                if(respuesta1.length > 0){
                    ...
                    $('#tabla-data-consulta').DataTable().rows.add(respuesta.datacobranzadets);

                    // Redibujar la tabla para mostrar los nuevos datos
                    $('#tabla-data-consulta').DataTable().draw();

                    totalizar(respuesta.datacobranzadets);
                }
            }
            if(funcion=='consultaanterior'){
    ...
    datos.clientes.forEach((cliente, index) => {
        ...
        if(cliente.datacobranzadets.length > 0){
            ...
            doc.autoTable({
                ...
                head: headRows(),
                body: bodyRows(cliente.datacobranzadets),
                theme: 'grid',
                styles: {
                    fontSize: 6, // Tamaño de letra para los encabezados
                },
...
function enviarcorreo(data){
    var ruta = '/reportdeudaclientesiscob/enviarcorreo';
    ajaxRequest(data.data1,ruta,'enviarcorreo');
}
...
function exportarExcel(datos) {
    ...
    datos.clientes.forEach(function(cliente) {
        ...
        /* if (cliente.vendedor_id != aux_vendedor_id){
            ...
            if (cliente.rut != aux_rut){
                arrayfusionarTitulos.push(count);
            }
       
            filainifusionar++;
            var filaExcel = [
                "RUT: " + formato_rutVar(cliente.rut) + " Razon Social: " + cliente.razonsocial
            ];
            count++;
            datosExcel.push(filaExcel);
...
$("#llenartabladatacobranza").click(function(e){
    cliente_id = $("#rut").attr("cliente_id");
    if(cliente_id != ""){
        llenartablaDataCobranza(cliente_id,cliente_id)
    }else{
        swal({
            title: 'Falta RUT Cliente',
            //text: "Presione F2 para buscar",
