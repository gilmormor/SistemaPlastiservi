...
function bodyRows(data) {
    ...
    data.forEach(row => {
        const rowArray = [row.NroFAV,fechaddmmaaaa(new Date(row.fecfact)),fechaddmmaaaa(new Date(row.fecvenc)),MASKLA(row.mnttot,0),MASKLA(row.Deuda,0)];
        ...
        body.push([row.NroFAV,fechaddmmaaaa(new Date(row.fecfact)),fechaddmmaaaa(new Date(row.fecvenc)),MASKLA(row.mnttot,0),MASKLA(row.Deuda,0)]);
    ...
    datos.clientes.forEach(function(cliente) {
        ...
        /* if (cliente.vendedor_id != aux_vendedor_id){
            ...
            cliente.datos.datacobranza.datosTodasFacDeuda.forEach(function(deuda) {
                ...
                var filaExcel = [
                    deuda.NroFAV,
                    fechaddmmaaaa(new Date(deuda.fecfact)),
                    fechaddmmaaaa(new Date(deuda.fecvenc)),
                    deuda.mnttot,
                    valorNumericoDeuda
                ];
