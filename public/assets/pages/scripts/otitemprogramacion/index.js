...
function configurarTabla(nombreTabla,url,serverSide) {
    ...
    tabla = $(nombreTabla).DataTable({
        ...
        'columns'     : [
            ...
            {data: 'otdet_obs'}, // 17
            {data: 'otdet_obs'}, // 18
            {data: 'clientebloqueado_descripcion',className:"ocultar"}, //19
            {data: 'oc_file',className:"ocultar"}, //20
            {data: 'oc_file',className:"ocultar"}, //21
            {data: 'updated_at',className:"ocultar"}, //22
            {defaultContent : 
        ...
        "createdRow": function ( row, data, index ) {
            ...
            aux_text = 
                `<input type="text" name="kgprod[]" id="kgprod${data.id}" class="form-control numerico" value="${MASKLA(data.kgprod,2)}" valor="${data.kgprod}" item="${data.id}" style="width: 100px;text-align:right;" maxlength="15"/>`;
            ...
            aux_text += `</select>`;
            $('td', row).eq(15).html(aux_text); // Insertar el select en la celda correspondiente
            ...
            aux_text += `</select>`;
            $('td', row).eq(16).html(aux_text); // Insertar el select en la celda correspondiente
            ...
            $('td', row).eq(22).addClass('updated_at');
            $('td', row).eq(22).attr('id','updated_at' + data.id);
            $('td', row).eq(22).attr('name','updated_at' + data.id);
