...
function insertarTabla(){
    ...
    var htmlTags = '<tr name="fila'+ aux_nfila + '" id="fila'+ aux_nfila + '" class="prod_id' + $("#producto_idM").val() + '">'+
			...
			'<td name="preciounitTD'+ aux_nfila + '" id="preciounitTD'+ aux_nfila + '" style="text-align:right">'+ 
				MASKLA($("#precionetoM").attr("valor"),3) + precioUnitSinCostoInsumos(aux_datosproducto) +  //MASK(0, $("#precionetoM").attr("valor"), '-##,###,##0.00',1)+
			'</td>'+
			'<td style="text-align:right;display:none;">'+ 
				...
				if(respuesta.length > 0){
					...
					if(datatemp.at_impreso == 1){
						...
						aux_botones.CrearProdImp = {
							text: "Crear Producto nuevo cliché",
							value: "CrearProdImp",
						};
						if ($("#btncrearAT").length === 0) {
							$("#contenedorBotonesAT").append(`
								<button type="button" id="btncrearAT" class="btn btn-primary" title="Presione este boton si esta seguro de  crear el Acuerdo Tecnico Temporal" onclick="seguirProcesoCrearAT()" data-dismiss="modal">
									Crear AT
								</button>
							`);
						}
...
$("#botonNewProdLote").click(function(event){
    //$(this).val("");
	if (!tablaProductoInicializadaSM) {
		configTablaProdSelectMult(); // ← AQUÍ recién se inicializa
		tablaProductoInicializadaSM = true;
	}
	...
	$.each(productosSeleccionados, function(index, producto) {
		if (!array_producto_ids.includes(producto.producto_id)){
			...
			var htmlTags = '<tr name="fila'+ aux_nfila + '" id="fila'+ aux_nfila + '" class="prod_id' + producto.producto_id + '">'+
					...
					'</td>'+
					'<td name="preciounitTD'+ aux_nfila + '" id="preciounitTD'+ aux_nfila + '" style="text-align:right">'+ 
						MASKLA(aux_precionetoM,3) + precioUnitSinCostoInsumos(aux_datosproducto) + //MASK(0, aux_precionetoM, '-##,###,##0.00',1)+
					'</td>'+
					'<td style="text-align:right;display:none;">'+ 
