$(document).ready(function () {
    ...
    let table =$('#tabla-data-consulta').DataTable({
        ...
        'processing'  : true,
        'serverSide'  : true,
        'ajax'        : "/opdetregprodtempaprobsup/listaropdetpage/" + data.data2, //$("#annomes").val() + "/sucursal/" + $("#sucursal_id").val(),
        "order": [[ 0, "desc" ]],
        'columns'     : [
            {
    ...
    {
        data = datosopdet();
        $('#tabla-data-consulta').DataTable().ajax.url( "/opdetregprodtempaprobsup/listaropdetpage/" + data.data2 ).load();
        //totalizar();
    });
